...
updateActualStatus () {
  ...
  actualStatus=$(curl --verbose --user admin:admin localhost:4502/system/console/bundles.json | jq --raw-output ".status")
}
...
sleep 5 # Needed to give the above operation time to come into force
